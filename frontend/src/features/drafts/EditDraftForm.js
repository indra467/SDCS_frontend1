...
const EditDraftForm = ({ draft, users }) => {
    ...
    const { isManager, isAdmin, isOperation_Employee, isBilling_Employee } = useAuth()
  ...
  const [current_location, setCurrent_location] = useState(draft.current_location);
  ...
  const [number_of_shifts, setNumber_of_shifts] = useState(draft.number_of_shifts);
  const [mobilization_charges, setMobilization_charges] = useState(draft.mobilization_charges);
  const [demobilization_charges, setDemobilization_charges] = useState(draft.demobilization_charges);
  const [SDCS_poc, setSDCS_poc] = useState(draft.SDCS_poc);
  const [delivery_deadline, setDelivery_deadline] = useState(draft.delivery_deadline);
  ...
  const[current_location2,setCurrent_location2]= useState(draft.current_location2);
  const[invoice_description,setInvoice_description]= useState(draft.invoice_description);
...
function downloadPDF3(pdf) {
 ...
 link.href = linksource;
  ...
  useEffect(() => {
    ...
    const onCurrent_location2Changed = (e) => setCurrent_location2(e.target.value);
    const onInvoice_descriptionChanged = (e) => setInvoice_description(e.target.value);
    ...
    const onSaveDraftClicked = async (e) => {
        if (canSave) {
            await updateDraft({ id: draft.id, user: userId,
                machine_no,
                current_location,
                c_name,
                site_location,
